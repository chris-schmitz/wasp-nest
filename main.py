...
NUMPIXELS = 12
neopixels = neopixel.NeoPixel(board.D1, NUMPIXELS, brightness=0.9, auto_write=False)
...
random_pixel_last_checked = 0
random_pixel_interval = 0.5
pixel = None
def get_random_pixel(now, changed_pixels):
    global random_pixel_last_checked, random_pixel_interval, pixel
    if now - random_pixel_last_checked > random_pixel_interval:
        global pixel
        random_pixel_last_checked = now
...
def slow_replace(now):
    ...
    if now - slow_replace_last_checked > slow_replace_interval:
        ...
        if len(changed) < NUMPIXELS - 1:
            ...
            pixel = get_random_pixel(now, changed)
...
#         try:
#             distance = round(
#                 map_range(round(ultrasonic.distance, 0), 0, 120, 0, NUMPIXELS)
#             )
#         except RuntimeError:
#             pass
